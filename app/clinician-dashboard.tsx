...
type EvaluationRecord = { id: string; patient_id: string; therapy_type: Therapy; responses: string[]; status: "Borrador" | "Validada"; created_at: string; updated_at: string };
...
export default function ClinicianDashboard() {
  ...
  const openEvaluation = (patient: Patient) => {
    setSelected(patient); setEvalType(patient.therapy); setInterview(false); setFinished(false); setQuestion(0); setAnswers(Array(11).fill("")); setEvaluation(null); setEvaluationLoading(true); setView("Evaluaciones");
    void fetch(`/api/evaluations?patient_id=${encodeURIComponent(patient.id)}`, { cache: "no-store" }).then(async (response) => {
      const body = await response.json();
      if (!response.ok) throw new Error(body.error || "No se pudo cargar la evaluación.");
      const saved = body.evaluation as EvaluationRecord | null;
      if (saved) { setEvaluation(saved); setEvalType(saved.therapy_type); setAnswers(saved.responses); setFinished(true); }
    }).catch((reason) => setError(reason instanceof Error ? reason.message : "No se pudo cargar la evaluación.")).finally(() => setEvaluationLoading(false));
  };
  ...
  return <main className="app-shell">
    ...
    <section className="content"><header className="topbar"><button className="mobile-logo" onClick={() => setView("Inicio")}>✦</button><div className="crumb">{view === "Ficha" && selected ? <><button onClick={() => setView("Pacientes")}>Pacientes</button><span>/</span><b>{selected.name}</b></> : <b>{view}</b>}</div><div className="top-actions"><button className="icon-btn" aria-label="Buscar">⌕</button><button className="primary" onClick={() => setModalOpen(true)}>＋ <span>Nuevo paciente</span></button></div></header>
      ...
      {view === "Evaluaciones" && <Evaluation patient={selected} eligiblePatients={patients.filter((patient) => patient.scheduled > 0)} onChoose={openEvaluation} interview={interview} setInterview={setInterview} type={evalType} setType={setEvalType} question={question} setQuestion={setQuestion} questions={questions} finished={finished} setFinished={setFinished} answers={answers} setAnswers={setAnswers} evaluation={evaluation} setEvaluation={setEvaluation} loading={evaluationLoading} />}
    </section>
...
function Evaluation({ patient, eligiblePatients, onChoose, interview, setInterview, type, setType, question, setQuestion, questions, finished, setFinished, answers, setAnswers, evaluation, setEvaluation, loading }: { patient: Patient | null; eligiblePatients: Patient[]; onChoose: (patient: Patient) => void; interview: boolean; setInterview: (value: boolean) => void; type: Therapy; setType: (value: Therapy) => void; question: number; setQuestion: (value: number) => void; questions: string[]; finished: boolean; setFinished: (value: boolean) => void; answers: string[]; setAnswers: (value: string[]) => void; evaluation: EvaluationRecord | null; setEvaluation: (value: EvaluationRecord | null) => void; loading: boolean }) {
  const [saving, setSaving] = useState(false);
  const [actionMessage, setActionMessage] = useState("");
  const [saveError, setSaveError] = useState("");
  const updateAnswer = (value: string) => setAnswers(answers.map((answer, index) => index === question ? value : answer));
  const chooseType = (value: Therapy) => { setType(value); setQuestion(0); setAnswers(Array(11).fill("")); setEvaluation(null); };
  const saveEvaluation = async () => {
    if (!patient) return;
    setSaving(true); setSaveError("");
    try {
      const response = await fetch("/api/evaluations", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ patient_id: patient.id, therapy_type: type, responses: answers }) });
      const body = await response.json();
      if (!response.ok) throw new Error(body.error || "No se pudo guardar la evaluación.");
      setEvaluation(body.evaluation as EvaluationRecord);
      setFinished(true);
      setInterview(false);
    } catch (reason) { setSaveError(reason instanceof Error ? reason.message : "No se pudo guardar la evaluación."); }
    finally { setSaving(false); }
  };
  const sheetText = () => {
    if (!patient) return "";
    const lines = questions.slice(0, 10).flatMap((item, index) => [`${index + 1}. ${item}`, answers[index] || "Sin respuesta registrada", ""]);
    return ["FISIOENCASA — FICHA DE EVALUACIÓN INICIAL", `Paciente: ${patient.name}`, `Modalidad: ${type}`, `Fecha: ${new Intl.DateTimeFormat("es-PE", { dateStyle: "long" }).format(new Date(evaluation?.updated_at || Date.now()))}`, "Estado: Borrador pendiente de validación profesional", "", ...lines, "OBSERVACIONES O NOTAS ADICIONALES", answers[10] || "Sin observaciones adicionales"].join("\\n");
  };
  const shareSheet = async () => {
    const text = sheetText();
    try {
      if (navigator.share) await navigator.share({ title: `Evaluación inicial — ${patient?.name || "Paciente"}`, text });
      else { await navigator.clipboard.writeText(text); setActionMessage("Ficha copiada. Ya puedes pegarla en la aplicación que prefieras."); }
    } catch (reason) {
      if (reason instanceof DOMException && reason.name === "AbortError") return;
      setActionMessage("No se pudo abrir el menú de compartir. Usa “Copiar ficha”.");
    }
  };
  const copySheet = async () => {
    try { await navigator.clipboard.writeText(sheetText()); setActionMessage("Ficha copiada de forma temporal."); }
    catch { setActionMessage("El navegador no permitió copiar. Puedes usar Imprimir / PDF."); }
  };
  ...
  if (loading) return <div className="page"><div className="card evaluation-loading">Cargando ficha de evaluación…</div></div>;
  if (!interview && !finished) return <div className="page eval-start"><p className="eyebrow">EVALUACIÓN INICIAL · {patient.name.toUpperCase()}</p><h1>Entrevista manual guiada</h1><p>Selecciona la modalidad y completa una respuesta por vez. Las respuestas se guardarán privadamente al generar la ficha.</p><div className="eval-options"><button className={type === "Física / Deportiva" ? "chosen" : ""} onClick={() => chooseType("Física / Deportiva")}><span>♧</span><b>Física / Deportiva</b><small>Lesión, dolor, limitaciones y retorno a la actividad.</small></button><button className={type === "Neurológica" ? "chosen" : ""} onClick={() => chooseType("Neurológica")}><span>◌</span><b>Neurológica</b><small>Evento neurológico, movilidad, equilibrio e independencia.</small></button></div><button className="primary large" onClick={() => { setInterview(true); setQuestion(0); setFinished(false); }}>Comenzar evaluación →</button><p className="simulation-note">En esta etapa no se graba audio ni se utiliza transcripción automática.</p></div>;
  if (finished) return <div className="page evaluation-sheet-page"><section className="evaluation-sheet" id="evaluation-sheet"><header className="sheet-header"><div><p>FISIOENCASA · DOCUMENTO CLÍNICO PRIVADO</p><h1>Ficha de evaluación inicial</h1></div><span>Borrador</span></header><div className="sheet-patient"><div><small>PACIENTE</small><b>{patient.name}</b></div><div><small>MODALIDAD</small><b>{type}</b></div><div><small>FECHA DE ACTUALIZACIÓN</small><b>{new Intl.DateTimeFormat("es-PE", { dateStyle: "medium" }).format(new Date(evaluation?.updated_at || Date.now()))}</b></div></div><div className="sheet-notice">Pendiente de revisión y validación profesional. No constituye por sí sola un diagnóstico médico.</div><div className="sheet-answers">{questions.slice(0, 10).map((item, index) => <article key={item}><h2><span>{index + 1}</span>{item}</h2><p>{answers[index] || "Sin respuesta registrada"}</p></article>)}</div><section className="sheet-observations"><h2>Observaciones o notas adicionales</h2><p>{answers[10] || "Sin observaciones adicionales"}</p></section></section><div className="sheet-actions"><button className="secondary" onClick={() => { setFinished(false); setInterview(true); setQuestion(0); }}>✎ Editar respuestas</button><button className="secondary" onClick={() => void copySheet()}>▣ Copiar ficha</button><button className="secondary" onClick={() => window.print()}>⌑ Imprimir / PDF</button><button className="primary" onClick={() => void shareSheet()}>↗ Compartir</button></div>{actionMessage && <p className="share-message" role="status">{actionMessage}</p>}</div>;
  const optional = question === 10;
  return <div className="page interview"><div className="interview-top"><button className="back" onClick={() => { setInterview(false); setFinished(Boolean(evaluation)); }}>← Salir</button><span>{type}</span><b>{question + 1} de {questions.length}</b></div><Progress value={((question + 1) / questions.length) * 100}/><p className="eyebrow">PREGUNTA {question + 1} {optional && <strong className="optional-tag">OPCIONAL</strong>}</p><h1>{questions[question]}{optional && <small className="optional-copy">Registra cualquier información relevante no incluida anteriormente.</small>}</h1><label className="manual-answer"><span>RESPUESTA MANUAL</span><textarea autoFocus value={answers[question]} maxLength={4000} onChange={(event) => updateAnswer(event.target.value)} placeholder={optional ? "Escribe observaciones o notas adicionales (opcional)…" : "Escribe aquí la respuesta del paciente…"}/><small>{answers[question].length} / 4000 caracteres</small></label>{saveError && <p className="form-error" role="alert">{saveError}</p>}<div className="interview-actions"><button className="secondary" disabled={!question || saving} onClick={() => setQuestion(question - 1)}>← Anterior</button><button className="secondary" disabled={saving} onClick={() => { updateAnswer("No aplica"); if (question < questions.length - 1) setQuestion(question + 1); }}>No aplica</button><button className="primary" disabled={saving} onClick={() => question === questions.length - 1 ? void saveEvaluation() : setQuestion(question + 1)}>{saving ? "Guardando…" : question === questions.length - 1 ? "Guardar y visualizar ficha" : "Siguiente →"}</button></div></div>;
}
