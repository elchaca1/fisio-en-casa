...
export default function ClinicianDashboard() {
  ...
  const openEvaluation = (patient: Patient) => { setSelected(patient); setEvalType(patient.therapy); setInterview(false); setFinished(false); setQuestion(0); setView("Evaluaciones"); };
  ...
  return <main className="app-shell">
    ...
    <section className="content"><header className="topbar"><button className="mobile-logo" onClick={() => setView("Inicio")}>✦</button><div className="crumb">{view === "Ficha" && selected ? <><button onClick={() => setView("Pacientes")}>Pacientes</button><span>/</span><b>{selected.name}</b></> : <b>{view}</b>}</div><div className="top-actions"><button className="icon-btn" aria-label="Buscar">⌕</button><button className="primary" onClick={() => setModalOpen(true)}>＋ <span>Nuevo paciente</span></button></div></header>
      ...
      {view === "Ficha" && selected && <PatientFile patient={selected} appointment={appointments.find((item) => item.patientId === selected.id) || null} tab={tab} setTab={setTab} onSchedule={() => setAppointmentPatient(selected)} onEvaluate={() => openEvaluation(selected)} />}
      ...
      {view === "Evaluaciones" && <Evaluation patient={selected} eligiblePatients={patients.filter((patient) => patient.scheduled > 0)} onChoose={openEvaluation} interview={interview} setInterview={setInterview} type={evalType} setType={setEvalType} question={question} setQuestion={setQuestion} questions={questions} finished={finished} setFinished={setFinished} />}
    </section>
...
function Evaluation({ patient, eligiblePatients, onChoose, interview, setInterview, type, setType, question, setQuestion, questions, finished, setFinished }: { patient: Patient | null; eligiblePatients: Patient[]; onChoose: (patient: Patient) => void; interview: boolean; setInterview: (value: boolean) => void; type: Therapy; setType: (value: Therapy) => void; question: number; setQuestion: (value: number) => void; questions: string[]; finished: boolean; setFinished: (value: boolean) => void }) {
